...
function updateScreen(fileText)
{
	document.getElementById("display").innerHTML = "<tr id=\"headings\">" +
												   "<th class=\"eventCell\" width=\"45%\"><label id=\"eventHeading\">Event</label></th>" +
												   "<th class=\"eventCell\" width=\"20%\"><label id=\"timeHeading\">Time</label></th>" +
												   "<th class=\"eventCell\" width=\"35%\"><label id=\"roomHeading\">Room</label></th></tr>";
	var fileArray = fileText.split(String.fromCharCode(35));
	document.getElementById('ticker').innerHTML = fileArray[0];  // Sets the top marque to the bannertext at the top of the file
	if(fileArray[1].length === 0)
	{
	...
	var column3;
	var tableArray;
	for(var i = 2; i < fileArray.length - 1; i++)  // Loops through the file starting at the third line
	{
		tableArray = fileArray[i];
		tableArray = tableArray.split(String.fromCharCode(36));  // Split the line by Record Seperator
		row = table.insertRow(table.rows.length);
		column1 = row.insertCell(0);
		column2 = row.insertCell(1);
		column3 = row.insertCell(2);
		column1.id = "eventCell";
		column2.id = "timeCell";
		column3.id = "roomCell"
